...
-- Keepsake videos are stored in the shared attachments bucket. Older installs
-- only allowed application/octet-stream, which made video/mp4 uploads fall into
-- the offline queue. Allow common mobile video MIME types while keeping the
-- octet-stream fallback compatible.
