...
export default function MediaLightbox({ item, onClose, showCloseButton = false }) {
  ...
  const uri = item?.uri || item?.media?.uri;
  const isVideo = item?.mime?.startsWith('video/') || item?.media?.kind === 'video' || item?.media?.mimeType?.startsWith('video/');
    ...
    <View style={styles.lightboxBg}>
      ...
      {(caption || dateStr) && (
        <BlurView intensity={60} tint="dark" style={[styles.lightboxMeta, { paddingBottom: insets.bottom + 12 }]}>
          ...
          {dateStr ? (
            <Text style={styles.lightboxDate}>{item.date ? formatDate(item.date) : dateStr}</Text>
          ) : null}
        </BlurView>
      )}
