# Legal Documents - Between Us

This directory contains all legal documents for the Between Us app.

---

## 📄 Documents

### Core Legal Documents

1. **[Terms of Service](./TERMS_OF_SERVICE.md)**
   - User agreement and rules
   - Account requirements
   - Subscription terms
   - Liability and disclaimers
   - Dispute resolution

2. **[Privacy Policy](./PRIVACY_POLICY.md)**
   - Complete privacy practices
   - Data collection and use
   - Security measures
   - User rights (GDPR/CCPA)
   - International compliance

3. **[FAQ](./FAQ.md)**
   - Frequently asked questions
   - Getting started guide
   - Feature explanations
   - Troubleshooting help
   - Contact information

### Supporting Documents

4. **[Legal Summary](./LEGAL_SUMMARY.md)**
   - Overview of all legal docs
   - Quick reference guide
   - Contact information
   - Compliance overview

5. **[Privacy Simplified](./PRIVACY_SIMPLIFIED.md)**
   - User-friendly privacy summary
   - Plain English explanations
   - For in-app display
   - Quick reference

---

## 🎯 Quick Reference

### For Users

**Need to know:**
- ✅ You must be 18+ to use Between Us
- ✅ Your intimate content is end-to-end encrypted
- ✅ We never sell your data
- ✅ You can delete your data anytime
- ✅ You control what you share with your partner

**Questions?**
- General: brittanyapps@outlook.com
- Privacy: brittanyapps@outlook.com
- Legal: brittanyapps@outlook.com

### For Developers

**Implementation:**
- Display Terms during onboarding
- Link to Privacy Policy in settings
- Show FAQ in help section
- Use simplified version for quick reference
- Update version numbers when docs change

**Required Displays:**
- Terms acceptance checkbox on signup
- Privacy Policy link in settings
- FAQ accessible from help menu
- Contact information in support section

---

## 📋 Compliance Checklist

### App Store Requirements
- ✅ Terms of Service available
- ✅ Privacy Policy available
- ✅ Age rating: 18+
- ✅ Data collection disclosed
- ✅ Encryption documented
- ✅ Contact information provided

### Legal Requirements
- ✅ GDPR compliant (EU/UK)
- ✅ CCPA compliant (California)
- ✅ COPPA compliant (18+ only)
- ✅ Terms clearly stated
- ✅ Privacy practices transparent
- ✅ User rights documented

### Best Practices
- ✅ Plain language used
- ✅ User-friendly formatting
- ✅ Accessible to all users
- ✅ Regular updates scheduled
- ✅ Version history maintained
- ✅ Contact methods provided

---

## 🔄 Update Process

### When to Update

**Terms of Service:**
- New features added
- Policy changes
- Legal requirements change
- Quarterly review

**Privacy Policy:**
- Data practices change
- New services added
- Legal requirements change
- Quarterly review

**FAQ:**
- New features launched
- Common questions arise
- User feedback received
- As needed

### How to Update

1. **Draft Changes**
   - Review current document
   - Identify necessary changes
   - Draft new version
   - Legal review (if major changes)

2. **Version Control**
   - Update version number
   - Update "Last Updated" date
   - Document changes in history
   - Archive previous version

3. **User Notification**
   - Email for material changes
   - In-app notification
   - Update log in app
   - Grace period before enforcement

4. **Implementation**
   - Update documents in app
   - Update website
   - Update app store listings
   - Train support team

---

## 📞 Contact Information

### Support Channels

**General Support**
- Email: brittanyapps@outlook.com
- Response: 24-48 hours

**Privacy Inquiries**
- Email: brittanyapps@outlook.com
- Data Requests: brittanyapps@outlook.com

**Legal Questions**
- Email: brittanyapps@outlook.com
- Business: brittanyapps@outlook.com

**Safety & Abuse**
- Email: brittanyapps@outlook.com
- Urgent: Immediate response

**Feedback**
- Email: brittanyapps@outlook.com
- Feature Requests: brittanyapps@outlook.com

---

## 🌐 Translations

### Current Languages
- English (primary)

### Coming Soon
- Spanish
- French
- German
- Portuguese
- More languages based on demand

**Request a language:** brittanyapps@outlook.com

---

## 📊 Transparency

### Annual Reports
We publish transparency reports covering:
- Government data requests (if any)
- Account terminations
- Security incidents
- Privacy improvements
- User rights requests

**Latest Report:** [Coming soon]

### Open Communication
- Regular blog posts about privacy
- Updates on security improvements
- User feedback incorporated
- Community engagement

---

## 🔐 Security

### Document Security
- Stored in version control
- Access controlled
- Change history tracked
- Regular backups
- Secure distribution

### User Data Security
- End-to-end encryption
- Client-side encryption (XSalsa20-Poly1305)
- TLS/SSL in transit
- Regular security audits
- Incident response plan

---

## 📱 In-App Integration

### Required Screens

**Onboarding:**
- Terms acceptance checkbox
- Privacy Policy link
- Age verification (18+)

**Settings:**
- Terms of Service link
- Privacy Policy link
- FAQ/Help link
- Contact support link
- Delete account option

**Help Section:**
- FAQ categories
- Search functionality
- Contact support
- Report issue

---

## 🎓 For Legal Review

### Review Checklist

**Terms of Service:**
- [ ] Age requirement clear (18+)
- [ ] Subscription terms accurate
- [ ] Liability limitations appropriate
- [ ] Dispute resolution defined
- [ ] Termination process clear
- [ ] Intellectual property protected

**Privacy Policy:**
- [ ] Data collection disclosed
- [ ] Use cases explained
- [ ] Security measures documented
- [ ] User rights listed
- [ ] Retention periods stated
- [ ] International transfers addressed

**Compliance:**
- [ ] GDPR requirements met
- [ ] CCPA requirements met
- [ ] App store guidelines followed
- [ ] Industry standards met
- [ ] Accessibility considered

---

## 📝 Document Standards

### Writing Style
- Plain language
- Short sentences
- Clear headings
- Bullet points
- Examples provided
- User-friendly tone

### Formatting
- Markdown format
- Consistent structure
- Table of contents
- Cross-references
- Version numbers
- Last updated dates

### Accessibility
- Screen reader compatible
- High contrast
- Clear hierarchy
- Alternative formats available
- Multiple languages (planned)

---

## 🚀 Launch Checklist

Before launching Between Us:

- [ ] All legal documents finalized
- [ ] Legal review completed
- [ ] Documents integrated in app
- [ ] App store listings updated
- [ ] Support team trained
- [ ] Contact emails active
- [ ] Backup procedures tested
- [ ] Compliance verified
- [ ] User testing completed
- [ ] Final approval obtained

---

## 📚 Additional Resources

### External Links
- [GDPR Official Site](https://gdpr.eu/)
- [CCPA Official Site](https://oag.ca.gov/privacy/ccpa)
- [Apple App Store Guidelines](https://developer.apple.com/app-store/review/guidelines/)
- [Google Play Policies](https://play.google.com/about/developer-content-policy/)

### Internal Resources
- Legal team contacts
- Compliance procedures
- Incident response plan
- User support protocols

---

**Last Updated:** February 22, 2026  
**Version:** 1.0  
**Status:** ✅ Ready for Production

---

**Questions about these documents?**  
Contact: brittanyapps@outlook.com
