const fs = require('fs');
const files = [
  'screens/SyncSetupScreen.js',
  'screens/PairingScanScreen.js',
  'screens/PairingQRCodeScreen.js',
];

for (let file of files) {
  let content = fs.readFileSync(file, 'utf8');

  // Convert static StyleSheet to dynamic
  content = content.replace(/const styles = StyleSheet\.create\(\{/, 'const createStyles = (colors, isDark) => StyleSheet.create({');

  // Inside the component
  content = content.replace(/const theme = useTheme\(\);/, 'const { colors, isDark } = useTheme();\n  const styles = createStyles(colors, isDark);');
  content = content.replace(/const \{ colors, isDark \} = useTheme\(\);/, 'const { colors, isDark } = useTheme();\n  const styles = createStyles(colors, isDark);'); // just in case
...
console.log("Fixed pairing and sync screens");